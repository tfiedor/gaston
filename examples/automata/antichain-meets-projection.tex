...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=5cm,
                    semithick, transform shape,initial text={}]
  ...
  {$\{1, 4\}$};
  \node[state,fill={rgb:black,1;white,4}]         (AB) [below right
  of=A] {$\{1, 2\}$}; 
  \node[state]         (ADE) [right of=AD]       {$\{1, 4, 5\}$};
  \node[state]         (ABC) [below right of=AB] {$\{1, 2, 3\}$};
  \node[state]         (ABDE) [below right of=AD]    {$\{1, 2, 4,
  5\}$}; 
  \node[state]         (ABCDE) [below right of=ABDE] {$\{1, 2,
  3, 4, 5\}$};
