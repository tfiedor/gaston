...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2.8cm,
  ...
  \path (A) edge [loop above] node {P: 0} (A)
  		...
  		0\textbar 1\textbar 1}} (D)
  		    edge [below]			  node {\makecell[l]{P: 1\\ X:
  		    0}} 
  	    (E) (E) edge [loop below] node {\makecell[l]{P: $\overline{X}$\\ X:
  	    $\overline{X}$}} (E); \node [below=5.5cm, align=flush center,text
  	    width=3cm] at (B) {
            \LARGE b) $\mathcal{A}_{P \not\subseteq X}$
        };
  \node [below=0.5cm, align=flush center,text width=3cm] at (B)
        {
