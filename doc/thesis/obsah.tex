...
4\}$ and $\{1, 4\}$ are simulated by the initial state $\{1\}$ so we don't need
...
Given formula $\phi$, constructing automaton $\mathcal{A}_\phi$ representing
all models of $\phi$ is not necessary, since representation of all models of formula
is not essential and usually any model or an invalid assignment suffices.
...
Formula $\phi$ is first transformed to the logical equivalent formula $\varphi$
in existential prenex form (see Section \ref{exPNF}):
\begin{equation*}
...
Supposing there are $m$	negations in the prefix of formula $\phi$, we create a
hierarchical family of WS$k$S formulae $\Phi = \{\varphi_0,\ldots,\varphi_m\}$
where
\begin{equation}
...
\subsection{Deciding WS1S}

The main concepts of the new approach to deciding WS$k$S is going to be
described first on WS$1$S formulae and in following section it will be
...
atomic formulae and their complements and using union and intersection
operations. Further we construct $\mathcal{A}_\varphi$ from automaton
$\mathcal{A}_\pi$ by successive applications of complementation $\gamma$ and
projection $\omega$ according to the prefix of $\varphi$ as defined in
\ref{preliminaries}.

As we outlined in Section \ref{wsks}, there is a problem with encoding of models
and using the projection on automata. After projection automaton corresponding
to the formula does not accept some of the valid models of formula, as described
on Figure \ref{models}. \textsc{MONA} (see Section \ref{mona}) solves this by
adjusting the final states of automaton by using the bread-first search
algorithm in linear time. 

However, this approach needs fully constructed automaton, so this cannot be used
in our on-the-fly algorithm. Instead we will precompute these states before
using any algorithm. 
...
\begin{lemma}
 Downward and upward closure of set $S$ is monotonic.
\end{lemma}
\begin{proof}
...
\begin{defz}
 We define the \emph{preset through zero track} as set of states such that
 \begin{equation}
 PRE_0(Q) = \{r\ |\ \exists q \in Q: r \overset{0^*}{\rightarrow} q\}.
 \end{equation}
...
and the hierarchical family of automata $\mathbb{A}$
\begin{lemma} For all $0 \leq i \leq m$, the language of $\mathcal(A)_i$ is
\begin{enumerate}
...
\begin{eqnarray}
 ...
 \Sigma_i\}\\
 I_{i+1} & = & \{I_i\}\\
 F_{i+1} & = & \overline{\downarrow\{Q\ |\ Q = PRE_0^*(P), P \in F_i\}}
\end{eqnarray}

\subsubsection{Constructing set of final states}\label{final-states}

\subsubsection{Algorithm of Decision procedure}
...
We use this set to examine whether there exists a satisfying example and
counterexample and use them to determine if a given formula is \emph{valid},
\emph{invalid, but satisfiable} or \emph{unsatisfiable}.

\begin{defz}
We say a formula is \emph{closed} if there occurs no free variable. We further
define a \emph{closure} of a formula $\varphi$ as formula $\phi$ such that
\begin{equation}
 \phi \overset{\mathit{def}}{=} \exists\mathcal{X}. \varphi
\end{equation}
where
\begin{equation}
 \mathcal{X} = \{X\ |\ X \in \mathtt{freeVars}_\varphi\}.
\end{equation}
...
denote this automat as $\tilde{\mathcal{A_\varphi}} = \{\tilde{Q},
\tilde{\Sigma}, \tilde{\delta}, \tilde{I}, \tilde{F}$. Note that since the
...
initial state of the automaton $\tilde{\mathcal{A}_\varphi}$ corresponding to
the closure of formula $\varphi$ is final, i.e.
\begin{equation}
 I_m \in \tilde{F_m}
\end{equation}
\end{lemma}
...
the initial state of automaton $\tilde{\mathcal{A}_{\neg\varphi}}$ corresponding
to the closure of formula $\varphi$ is final.
\end{lemma}
...
on Lemmas \ref{exists-example} and \ref{exists-counter-example} and computed list of final
states of automaton corresponding to the formula. The more detailed
...
\begin{algorithm}[hb!]
		\SetKwFor{For}{foreach}{do}{}
		\SetKwRepeat{Repeat}{repeat}{until}
		\KwIn{Automaton $\mathcal{A}_0$, a set
		$\{\mathcal{X}_1,\ldots,\mathcal{X}_{m+1}\}$ of sets of second-order
		variables}
		\KwOut{\texttt{UNSAT} if formula is unsatisfiable, \texttt{VALID} if formula
		is valid, \texttt{SAT} otherwise}
		\BlankLine
		$F_m = \{q\ |\ L(q) \cap (0^k)^* \neq \emptyset\}$\;
		$example = a_1\ldots a_n$, $a_i \in \Sigma_m$, for $0 < i < n$, $\exists q_0
		\in I_m: q_0 \overset{example}{\longrightarrow} q_f$, $q_f \in F_m$\;
		$counter-example = b_1\ldots b_k$, $b_i \in \Sigma_m$, for $0 < i < k$,
		$\exists q_0 \in I_m: q_0 \overset{example}{\longrightarrow} q_f$, $q_f \in
		Q_m \setminus F_m$\;
		\uIf{example = $\emptyset$}{
		 \Return \texttt{UNSAT}\;
		}
		\uElseIf{example $\neq \emptyset$ \&\& counter-example $= \emptyset$}{
		 \Return \texttt{VALID}\;
		}
		\Else{
		 \Return \texttt{SAT}\;
...
\texttt{libvata} \cite{vata} written in C++ was chosen for being highly
efficient and open source library that exploits some of the recent developments
of algorithm for NTA manipulation. Its main usage is in fields of formal verification, but can be effectively used in other domains as well. 
...
transition. 
 ...
 The main algorithm for deciding WS$k$S formulae is implemented w.r.t the formal
 desinged as outlined in Section \ref{ours}. In that section it was also proved
 that it is correct.
 
 The input of procedure is a set of second-order quantified variables
 $\{\mathcal{X}_1,ldots,\mathcal{X}_{m+1}\}$ obtained from prefix of the input
 formula and an NTA corresponding to the matrix of the formula.
 
 Further for each level $0 \leq i \leq m$ we construct a \emph{transition cache}
 that maps states of level $i$ to the MTBDD corresponding to their transition
 relation $\delta_i$, i.e. it implements a function $transCache_i$ such
 that
 \begin{equation}
  transCache_i : Q_i \rightarrow (\Sigma_i \rightarrow 2^{Q_i})
 \end{equation} where $transCache_0$ is pre-filled with the transition relation
 of matrix automaton $\delta_0$. If a value for the looked-up key $q_i \in Q_i$
 is not in the cache, then we define $transCache_i(q_i) \mapsto \bot$.
 
 On several levels the constructed automaton corresponding to the input formulae
 is minimised by function \texttt{minimiseSet(S)}, where $S$ is a set of states.
 This optimization does not need to be concrete for algorithm to work, so it
 could be for example implemented as classic Myhill-Nerode equivalence reduction
 \cite{MH}. However most of these minimisations requires a deterministic
 automaton to be constructed first so it is not suitable for our procedure.
 Instead we are using the simulation relation as defined in Section \ref{sim}.
 It must hold that $$L(minimiseSet(S)) = L(S)$$.
