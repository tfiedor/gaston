...
the family of automata $\mathbb{A} = \{\mathcal{A}_0,\ldots,\mathcal{A}_m\}$ as follows:
 ...
 Note that the automaton for case $i$ thus correspond to the following
 automaton:
...
{\color{red} Do we even need this???}
...
\begin{defz}
 ...
 \begin{eqnarray}
  \leq_0 & = & id\\
  A \leq_i B & \Leftrightarrow & \forall a \in A \exists b \in B: a \leq_i b\\
 \end{eqnarray}
\end{defz}

\begin{lemma}
 For all $0 \leq i \leq m$ it holds that
 \begin{equation}
  \leq_i \subseteq \leq_i^L \Rightarrow \leq_{i+1} \subseteq \leq_{i+1}^L
 \end{equation}
\end{lemma}
\begin{proof}
...
\begin{algorithm}[hb!]
		...
		$\{\mathcal{X}_1,\ldots,\mathcal{X}_{m}\}$ of sets of second-order
		variables, a set of free variables $\mathcal{X}_{m+1}$}
		\KwOut{\texttt{UNSAT} if formula is unsatisfiable, \texttt{VALID} if formula
		is valid, \texttt{SAT} otherwise}
		\BlankLine
		existsSatExample = \texttt{IsFinal($I_m$, m)}\;
		existsUnSatExample = \texttt{IsFinal($I_{m+1}, m+1$)}\;
		\BlankLine
		{\color{red}Yes this is chaotic. Basically the SAT example can be computed
		by closing the formula and checking if $I_m$ is final. However for UNSAT
		example we have to do the complement and then close it, so maybe just adding
		the one more level? Is it enough? Or should I specify it another way?}
		\BlankLine
		\uIf{not existsSatExample}{
		\Return
		\texttt{UNSAT}\; }
		\uElseIf{existsSatExample and not existsUnSatExample}{
		 \Return \texttt{VALID}\;
		}
		\Else{
		 \Return \texttt{SAT}\;
...
\begin{function}[h]
  ...
  \caption{IsStateFinal(macro-state $Q$, unsigned $\mathit{level}$)}
  \KwOut{TRUE if macro-state $Q$ is final on $\mathit{level}$, FALSE otherwise}

  \BlankLine
  {\color{red} Very rough draft of how it should work. It is similar to the
  formal description from the tabula that we discussed. Didn't state how the
  Succ will be computed for now. First I'd like to solve this function.}
  \BlankLine
  ...
  \Else{
  	...
  	\While{worklist $\neq \emptyset$}{
  	  q = worklist.pop()\;
  	  \uIf{IsStateFinal(q, level-1)}{
  	    \Return False\;
  	  }
  	  \Else{
  	    worklist = worklist $\cup Succ_0$(q, level-1)\;
  	  }
  	}
  	\Return True\;
  }
