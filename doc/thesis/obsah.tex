...
performed in the virtualized environment with operating system Linux Ubuntu
12.10 with 4096\ MB of memory and one virtual processor natively running on a
laptop computer with a 8\ GB RAM memory and dual core processor running on 2.5
GHz.
...
approach.
The run tests were measuring the speed and number of generated states of
decision procedure depending on the size of the input formula and number of
quantifier alternations. The other tests are examining the impact of used
optimizations on the performance of implementation, like the cache hit-miss
...
Our first batch of experiments done with our implemented tool were carried out
on specific artificially created parametric formulae in the following Horn form
\cite{horn}:
\begin{equation}
...
Also it was further examined that \textsc{MONA} is able to only decide the
formulae up to parameter $n = 15$ due to insufficient memory for storing MTBDDs.
In results it is shown that the logical approach designed in
...
\subsection{Comparison with \textsc{MONA}}

\subsection{Impact of used optimizations}

In this subsection we will discuss the impact of optimizations developped during
process on the performance of implementation.
