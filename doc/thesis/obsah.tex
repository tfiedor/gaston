...
$Post(p) = \{p'\in Q\ |\ \exists a \in \Sigma: (p, a, p') \in \delta\}$.
...
\begin{algorithm}[ht!]
		...
		\KwIn{A state $I$ of automaton $\mathcal{A}$, a simulation relation
		$\leqq \subseteq A^{\subseteq}$ on states of
		$\mathcal{A}$, a successor function $\delta$ and a predicate
		\texttt{IsAccepting(F)} that decides whether state $F$ is final}
		...
		\BlankLine
  \myproc{\upshape \texttt{CheckForReachableState}(\textbf{state} I, level i,
  \textbf{post function} $\delta$, \textbf{pred} \texttt{IsAccepting})}{
    \nl\uIf{\upshape \texttt{IsAccepting}(I)}{
    	\nl\Return \textsc{TRUE}\;}
		...
		\nl\While{$Next \neq \emptyset$}{
		  ...
		  $Processed$\;
			 \nl \uIf{\upshape \texttt{IsAccepting}(R)}{\nl \Return \textsc{TRUE}\;}
			\nl \For{$P \in \delta(R)$}{
...
G$, written $\omega_H(E)$, as $\omega_H(E) = \{\omega_H(f)\ |\ f \in E\}$.
...
\end{lemma}
\begin{proof}
{\color{red} will we even need this?}
\end{proof}
...
\begin{equation}
 \mathit{PRE}_0(Q) = \{r \in Q\ |\ \exists q \in Q: r
...
\begin{eqnarray}
 ...
 \delta^0 & = & \{p \overset{\overline{0}}{\longrightarrow} q\ |\ p, q \in Q^0\}
\end{eqnarray}
\end{defz}
...
algorithm as described in this chapter. It comes from 

\begin{algorithm}[hb!]
		\SetKwFor{For}{foreach}{do}{}
		\SetKwRepeat{Repeat}{repeat}{until}
		\KwIn{Initial state $I_m$ of automaton $A_{\neg\varphi_m}$}
  ...
  \nl \uIf{i = 0}{
  	...
  	\SetEndCharOfAlgoLine{}
  		\nl \For{$p \in P$}{
  			\nl \uIf{\upshape \texttt{CheckForReachableState}(p, i-1, $\delta^0_{i-1}$,
  			($\lambda$ q.
...
further extend this procedure to an arbitrary $k$.
...
representation.
 ...
 whether the state on level $0 \leq i \leq m$ is final or not. Since we are
 constructing a complement of an automaton on level $i$, the state is final iff
 all of its child states are non-final or there exists a successor over the zero
 tracks that is final (due to projection). While generating successors
 we can also prune some of the states if they are in relation $\leq_i$ as
 defined previously.
...
\begin{function}[h!]
		...
		\nl \Else{
			\nl\For{$q \in Q$}{
				\nl \uIf{\upshape \texttt{StateIsFinal}(q, m-1)}{
					\nl isFinal := \textsc{FALSE}\;
					\nl \textbf{break}\;
				}
				\nl \Else{
					\nl succ := \texttt{buildSuccessorTree}(q, m)\;
					\nl \textbf{apply1} succ ($\lambda$ x. \textbf{if} x is reachable by
					low successors only in BDD \textbf{then} workset := workset $\cup$ x)\;
				}
			}
			\nl isFinal := \textsc{TRUE}\;
		}
		\nl \texttt{StateCache}[q] = isFinal\;
		\nl \Return isFinal\;
		\caption{StateIsFinal(state q, level m)}\label{impl-state-is-final}
\end{function}
 
 The main Algorithm \ref{impl-main} is based on the workset algorithm. Since we
 are working with closed formulae, at the last level of determinization in
 the defined family of automata we are looking for a non-final state using the
 function \ref{impl-state-is-final}. If
 ...
 $\leq_i$ relation, i.e. if the successors $q$ of state $p$ is $\leq_i$-smaller
 we discard it.
...
\begin{algorithm}[h!]
		...
		\nl \While{$\exists q_m \in workset$}{
			...
			\nl\uIf{\upshape \texttt{StateIsFinal}($q_m$, level) = \textsc{FALSE}}{
				\nl \Return \textsc{TRUE}\;
			}
			\nl\Else{
				\nl succ := \texttt{buildSuccessorTree}($q_m$, level)\;
