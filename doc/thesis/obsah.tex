...
\emph{union} of languages $L(\mathcal{A})$ and $L(\mathcal{B})$  $$ \mathcal{A}
\cup \mathcal{B} = (Q_\mathcal{A} \cup Q_\mathcal{B}, \Sigma, \delta_\mathcal{A}
\cup \delta_\mathcal{B}, I_\mathcal{A} \cup I_\mathcal{B}, F_\mathcal{A} \cup
F_\mathcal{B})$$
...
$L(\mathcal{B})$ $$\mathcal{A} \cap \mathcal{B} = (Q_\mathcal{A} \times
Q_\mathcal{B}, \Sigma, \delta, I_\mathcal{A} \times I_\mathcal{B}, F_\mathcal{A}
\times F_\mathcal{B})$$ where $$\delta = \{(p_1, p_2)
...
accepting the \emph{complement} of $L(\mathcal{A})$ $$\overline{\mathcal{A}} =
...
\emph{intersection} of languages $L(\mathcal{A})$ and $L(\mathcal{B})$  $$\mathcal{A} \cap \mathcal{B}
= (Q_\mathcal{A} \times Q_\mathcal{B}, \Sigma, (F_\mathcal{A} \times
Q_\mathcal{B}) \cup (Q_\mathcal{A} \times F_\mathcal{B}), \delta_\mathcal{A}
\times \delta_\mathcal{B})$$ where
 \begin{eqnarray*}
 \delta_\mathcal{A} \times \delta_\mathcal{B} & = & \{
 ((q^1_1,q^2_1),\ldots,(q^1_n,q^2_n)) \overset{f}{\longrightarrow} (q^1, q^2)\\
...
of languages $L(\mathcal{A})$ and $L(\mathcal{B})$ $$\mathcal{A} \cap
...
$L(\mathcal{A})$ $$\overline{\mathcal{A}} = (Q, \Sigma, Q - F, \delta)$$.
\end{proof}
...
(T_\Sigma)^n$ such that the language $$\{[t_1,\ldots,t_n]\ |\ (t_1,\ldots,t_n)
\in R\}$$ is accepted by a tree automaton on the alphabet $\Sigma_\bot^n$.
\end{defz}
...
defined by $$ R_i(t_1,\ldots,t_{n-1}) \Leftrightarrow \exists t \in T_\Sigma .
...
\begin{defz}
 ...
 (T_\Sigma)^{n+1}$ defined by $$ R^i(t_1,\ldots,t_{i-1},t,t_i,\ldots,t_n)
...
$$s_1,\ldots,s_n,S_1,\ldots,S_m \models \psi(x_1,\ldots,x_n,X_1,\ldots,X_m)$$ if
and only if $$\{s_1\},\ldots,\{s_n\},S_1,\ldots,S_m \models_2
T(\psi)(X_{x_1},\ldots,X_{x_n}, X_1,\ldots,X_m)$$.

 Conversely, there is a
translation $T'$ from the restricted syntax to WS$k$S such that $$S_1,\ldots,S_m
\models T'(\psi)(X_1,\ldots,X_m)$$ if and only if $$S_1,\ldots,S_m \models_2
\psi(X_1,\ldots,X_m)$$.
	\end{prop}
	\begin{proof}
...
$$\phi = Q_1X_1Q_2X_2\ldots Q_nX_n\ .\ \psi(\mathds{X})$$ where  $Q_i \in
\{\exists,\forall\}$, $1 \leq i \leq n$, and $\psi$ is a quantifier-free formula
...
form} (abbreviated as $\exists$PNF), if and only if $$\rho = \exists
\mathcal{X}_{m+1}\neg\exists \mathcal{X}_m\ldots\neg\exists
\mathcal{X}_2\neg\exists \mathcal{X}_1.\psi(\mathds{X})$$ where
...
\begin{defz}
 ...
 that $$(S_1,\ldots,S_n) \in L \text{ if and only if } S_1,\ldots,S_n \models
 \psi.$$
...
\begin{eqnarray*}
  ...
  (q'', q) & \overset{00}{\longrightarrow} & q''
\end{eqnarray*}
Note that the automaton for $X = Y2$ is obtained similarly by chaning $(q', q) &
...
\begin{itemize}
 ...
 for connective $\vee$ byt by using \emph{intersection} of automata.
...
which removes quantifiers, where they are not useful, as following: $$\exists X
. \phi \Leftrightarrow \phi[T/X]$$ provided that $\phi \Rightarrow X = T$ is
valid and $T$ is some term satisfying $freevar(T) \subseteq freevar(\phi)$,
where $freevar$ denotes the set of free variables of subformula. This is further
restricted to different rewrite rule: $$ \exists X_i . \phi \Leftrightarrow
\phi[X_j/X_i]$$ provided that $\phi \equiv \ldots \wedge X_i = X_j \wedge
