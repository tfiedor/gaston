...
Given formula $\phi$, constructing automaton $\mathcal{A}_\phi$ repr esenting
...
Formula $\phi$ is first transformed to the logicall equivalent formula $\varphi$
...
Testing the satisfiability of $\varphi$ is equal to testing \emph{non-emptiness}
of $\mathcal{A}_m$, which can be done on-the-fly by traversing the state space
and trying to find a final state $q_f \in F_m$. This search can be optimised by
prunin states that are subsumed by other states over the entire hierarchical
family of automata.

\begin{lemma}
Let $\leq_i \subseteq Q_i \times Q_i$, for $0 \leq i \leq m-1$, be a relation on
the states of automaton $\mathcal{A}_i$ that implies inclusion of languages of
states, i.e. $p \eq_i q \Rightarrow L_{\mathcal{A}_i}(p) \subseteq
L_{\mathcal{A}_i}(q)$. Then $\forall p, r \in Q_i$ and $S \subseteq Q_i$ it
holds that
\begin{equation}
 p \leq_i r \Rightarrow L_\mathcal{i+1}(\{p, r\} \cup S) = L_\mathcal{i+1}(\{r\}
 \cup S)
\end{equation} where $\{p, r\}\cup S$, $\{r\}\cup S \in Q_{i+1}$ are states of
the automaton $\mathcal{A}_{i+1}$.
\end{lemma}

\begin{defz}
 Given an automaton $\mathcal{A} = (Q, \Sigma, \delta, I, F)$ and a binary
 relation $\preceq \subseteq Q \times Q$ on the states of $\mathcal{A}$ we
 defined the operator $\sqsubseteq: (Q\times Q) \rightarrow (2^Q\times 2^Q)$
 applied to binary relations over $Q$ as follows:
  \begin{equation}
   X \sqsubseteq(\preceq) Y \overset{\mathit{def}}{\Longleftrightarrow} \forall
   y \in Y \exists x \in X: x \preceq y
  \end{equation}.
  Further we can define the power of $\sqsubseteq(\preceq)$ as 
  \begin{eqnarray}
   \sqsubseteq^0_\preceq & = & \preceq\\
   \sqsubseteq^{n+1}_\preceq & = & \sqsubseteq(\sqsubseteq^n_\preceq)
  \end{eqnarray}.
  We can further extent this notion to high levels as stated in following lemma.
  \begin{lemma}
   Let $\leq_i \subseteq Q_i \times Q_i$, for $0 \leq i \leq m-1$, be a relation
   on the states of $\mathcal{A}_i$. If $\leq_i$ implies language inclusion on
   the states of $\mathcal{A}_i$ then $\sqsubseteq_{\preceq_i}$ implies language
   inclusion on the states of $\mathcal{A}_{i+1}$, i.e. 
   \begin{align}
    \text{if} p \leq_i r \Rightarrow L_{\mathcal{A}_i}(p) \subseteq
    L_{\mathcal{A}_i}(r) \text{then}\\
    P \sqsubseteq_{\preceq_i} R \Rightarrow L_{\mathcal{A}_{i+1}}(P) \subseteq
    L_{\mathcal{A}_{i+1}}(R)
   \end{align}
  \end{lemma}
\end{defz}
