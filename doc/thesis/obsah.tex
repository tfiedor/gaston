...
\begin{algorithm}[hb!]
		...
		$\mathcal{A}$, procedure for computing $Post$ set of the state $Q$ $Post(Q)$
		and a predicate $IsAccepting(F)$ that decides whether state $F$ is final}
		\KwOut{\textsc{TRUE} if there $\exists$ a final state in $\mathcal{A}$}
		\BlankLine
    \lIf{IsAccepting(I)}{\textbf{return} \textsc{FALSE}}
		$Processed := \emptyset$\;
		$Next := \{(I\}$\;
		\While{$Next \neq \emptyset$}{
		  Pick and remove a macro-state $R$ from $Next$ and move it to $Processed$\;
			\For{$P \in Post(R)$}{
			 \lIf{IsAccepting(P)}{\textbf{return} \textsc{FALSE}}
...
\begin{algorithm}[hb!]
		...
		\BlankLine
		\uIf{\texttt{IsStateAccepting}($I_m$, m) = \textsc{TRUE}}{
		  \Return \textsc{TRUE}\;
		}
		\Else{
		  \Return \textsc{FALSE}\;
  ...
  \myproc{IsStateAccepting(state P, level i)}{
  	...
  	\SetEndCharOfAlgoLine{}
  	  Call Algorithm \ref{universality} with following parameters:\;
  	   \hspace{1cm}p,\;
  	   \hspace{1cm}i - 1,\;
  	   \hspace{1cm}$Post^0_{i-1}$,\;
  	   \hspace{1cm}\\$\lambda$ q.  IsStateAccepting(i-1, q).\;
  	\SetEndCharOfAlgoLine{;}
  	  \uIf{All invocations of Algorithm \ref{universality} returns
  	  \textsc{FALSE}}{
  	    \Return \textsc{TRUE}\;
  	  } 
  	  \Else{
