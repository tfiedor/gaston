...
$\varphi$ in existential prenex form (see Section \ref{exPNF}):
\begin{equation*}
...
The main concept of the new approach to deciding WS$k$S is going to be
...
operation were defined in \ref{preliminaries}.

As we described in Section \ref{wsks}, there is a problem with encoding of
all models and using the projection operation on automata. After the projection
the automaton corresponding to the formula does not accept some of the valid
models of formula, as described on Figure \ref{models}. \textsc{MONA} (see Section \ref{mona}) solves this by
adjusting the final states of automaton by using the bread-first search
algorithm in linear time after each projection in formula. 
...
\{\mathcal{A}_0,\ldots,\mathcal{A}_m\}$ as follows:
 ...
 Note that the automaton for case $i$ thus correspond to following automaton:
...
\subsection{Computing predecesors of set}
...
\begin{equation}
 \widetilde{I}_m \in \widetilde{F}_m \Rightarrow \exists \mathcal{M}:
...
\begin{algorithm}[hb!]
		...
		\uElseIf{example $\neq \emptyset$ and cnt-ex $= \emptyset$}{
		 \Return \texttt{VALID}\;
		}
		\Else{
		 \Return \texttt{SAT}\;
