...
resulting deterministic automaton. MONA tool is thus heavily optimized for the
use of deterministic finite (tree) automata, as we introduced in
\ref{monasecrets}.
...
fact that there exist optimized libraries for their usage, like VATA
...
Here we propose to use algorithm based on the principle of antichains
\cite{tacas}, as defined in the following section, and search for an accepting
or non-accepting state \emph{on-the-fly} without constructing the automaton in
the first place.

\section{Antichain based universality testing}

We will give a brief introduction to universality testing for non-deterministic
finite word automata and tree automata by an algorithm that uses a combination
of the simulation-based and the antichain-based approach \cite{tacas}.

\begin{defz}
A~\emph{simulation} on a finite automata $\mathcal{A} = (Q, \Sigma, \delta, I,
F)$ is a relation $\preceq \subseteq Q \times Q$ such that $p \preceq r$ only if
...
(then this is the so called antichain approach).

The second optimization is based on the observation that $L_\mathcal{A}(P) =
L_\mathcal{A}(P - \{p_1\})$ if there is some $p_2 \in P \setminus \{p_1\}$ such
...
\begin{algorithm}[hb!]
		...
		$\mathcal{A}$, successor function $\delta$ and a predicate $IsAccepting(F)$ that decides whether state $F$ is
		final}
		...
		\BlankLine
  \myproc{CheckForReachableState(state I, level i, post function $\delta$, pred
  IsAccepting)}{
    \uIf{IsAccepting(I)}{\Return \textsc{TRUE}\;}
		$Processed := \emptyset$\;
		$Next := \{I\}$\;
		\While{$Next \neq \emptyset$}{
		  Pick and remove a macro-state $R$ from $Next$ and move it to $Processed$\;
			\For{$P \in \delta(R)$}{
			 \uIf{IsAccepting(P)}{\Return \textsc{TRUE}\;}
...
and no accepting macro-state has been found, one of the macro-states from $Next$
...
\begin{lemma}\label{search-is-correct}
 Algorithm \ref{universality} returns \textsc{TRUE} iff it reaches an accepting
 state.
\end{lemma}
\begin{proof}
...
accepting state 7 states are generated as depicted at Figure
...
to expand states any further. Since we didn't reach an accepting state, we can
conclude that language of automaton $\mathcal{A}_\varphi$ is not universal and
formula is thus invalid.
\end{example}
...
\section{Deciding WS$k$S with NFAs}
...
\begin{equation}
 ...
 \varphi_i.
\end{equation}
The relation between this family of formulae $\Phi$ and formula $\varphi$ is
depicted as follows:
\begin{equation}
...
We will use $\Gamma(G)$ to denote the alphabet used in automaton defined as the
set of functions $Theta(G) = \{f : G \rightarrow \{0, 1, \bot\}\}$, with the special case for $G
= \emptyset$ defined as $\Gamma(\emptyset) = \{\#\}$. Elements of this set can
be viewed as strings of fixed lenght $|G|$ over the alphabet $\{0, 1, \bot\}$ where
each position in the strings is associated with exactly one element of G.

Now given the symbol $f \in \Gamma(G)$ and a set $H \subseteq G$ the
\emph{projection} of $f$ over set $H$ written as $\omega_H(f)$, can be defined
as the restriction of the function $f$ over the set $H$, i.e. $\omega_H(f) =
f|_{G\setminus H}$. This can be further extended to the sets of symbols. Given a
set $E \subseteq \Gamma(G)$, we define the projection over a set $H \subseteq G$
written as $\omega_H(E)$, as $\omega_H(E) = \{\omega_H(f)\ |\ f \in E\}$. Given
a tree $t : \mathbb{N}^* \rightarrow \Gamma(G)$ the projection of $t$ over $H
\subseteq G$ is defined as $\omega_H(t) = \{(n, \omega_H(f))\ |\ (n, f) \in
t\}$. NOte that $\omega_H(t) : \mathbb{N}^* \rightarrow \Gamma(G\setminus H)$,

Now for a language $L \subseteq \Gamma(G)^*$, the projection of $L$ over $H$,
written as $\omega_H(L)$, is defined as $\omega_H(L) = \{\omega_H(f)\ |\ f \in
L\}$. Note that $\omega_H(L) \subseteq \Gamma_{G\setminus H}^*$.

\begin{lemma} Given the alphabet $\Gamma(G)$, $H \subseteq G$ and $X, Y \in
\Gamma_G^*$ it holds that
\begin{equation}
...
\begin{defz}
 ...
 \begin{equation}
  \delta_\gamma = \{R \overset{a}{\rightarrow}\ S |\ \exists r \in R, s \in S: r
...
\begin{equation}
 I_m \subseteq F_m \Leftrightarrow \models \varphi
\end{equation}
\end{lemma}
...
\begin{defz}
Given the automaton $\mathcal{A} = (Q, \Sigma, \delta, I, F)$ we define the
automaton containing only zero tracks reachable from initial state as the
automaton $\mathcal{A}^0 = (Q^0, \Sigma, \delta^0, I, Q^0)$ where
\begin{eqnarray}
 Q^0 & = & I \cup \{q\ |\ \exists w \in \{(0^k)\}^*: q
 \overset{w}{\longrightarrow} f, f \in I\}\\
 \delta^0 = \{p \overset{(0^k)}{\longrightarrow} q\ |\ p, q \in Q^0\}
\end{eqnarray}
\end{defz}

Note that all the states are final due to the adjustment of states to conform
the problem with model encoding.
...
\begin{algorithm}[hb!]
  ...
  \myproc{IsStateAccepting(state P, level i)}{
  	...
  	\nl CheckForReachableState(p, i-1, $\delta^0_{i-1}$, $\lambda$ q.
...
{\color{red} this is where i'll surely need some help. Used the part of the
draft and partly I overwrote it to correspond more with the definitions from
previous section}

In previous section we introduced the concept of the algorithm on WS$1$S logic
using the nondeterministic finite automata (or unary tree automata). Now we will
further extend this procedure to arbitrary $k$.
...
\end{equation} is a nondeterministic finite tree automaton corresponding ot the
...
\end{equation}
is a tree automaton corresponding to the formule $\varphi_{i+1}
\overset{\mathit{def}}{=} \neg\mathcal{X}_{i+1}\varphi_i$ where
\begin{eqnarray}
 Q_{i+1} & = & 2^{Q_i}\\
 \Sigma_{i+1} & = & \omega_{\mathcal{X}_{i+1}}(\Sigma_i)\\
 \delta_{i+1} & = & \{((R_1),\ldots,R_t), \omega_{\mathcal{X}_{i+1}}(f), S)\}\\
 S & = & \{s \in Q_i\ |\ \exists r_1 \in R_1,\ldots,r_t \in R_t.
 ((r1,\ldots,r_t), f, s) \in \delta_i\}\\
 F_{i+1} & = & \{R \in Q_{i+1}\ |\ R \cap F_i = \emptyset\}
\end{eqnarray}
...
{\color{red} What else is needed? The rest of the draft part?}
...
The frontend of the application is reused and slightly modified parser of tool
\textsc{MONA} that is based on \texttt{yacc} \cite{yacc} parsing tool. During
the parsing of input specification, a symbol table is filled with used first or
second-order variables and predicates/macros. Mapping of the variables to the
BDD tracks is constructed in several possible ways. Either it corresponds to the
place of the definition during parsing process, or can be randomly shuffled or
matches the chain of the quantifiers from prefix, i.e. during projection of the
track the reordering of the tree takes less time.
...
\begin{center}
 ...
 \caption{Transformation of data through the decision process}
 \end{center}
\end{figure}
...
transition. 
 ...
 In Chapter \ref{ours} we introduced the formal concepts of deciding WS$k$S
 with non-deterministic automata and designed the algorithm for testing validity
 of given formula $\varphi$. In this section we will closely decribed the
 implementation of this formal algorithm in language c++.
 
 For each level $0 \leq i \leq m$ we define two caches that will store the
 already computed results to achieve better results. In
 case we get the cache miss, the cache returns value $\bot$ instead.
 $BDDCache_i$ is a cache that maps states of level $i$ to their
 corresponding BDDs representing their transition relations $\delta_i$, i.e.
 \begin{equation}
  transCache_i : Q_i \rightarrow (\Sigma_i \rightarrow 2^{Q_i}).
 \end{equation}
 
 Further we define the cache for storing the finality of states
 $StateCache_i$ that maps macrostates to their finality, i.e.
 \begin{equation}
  StateCache_i : Q_i \rightarrow \{Final, NonFinal\}
 \end{equation}
 
 The main core of the deciding is Function that is checking whether the state on
 level $0 \leq i \leq m$ is final or not. Since we are constructing a
 complement of automaton on level $i$ the state is final iff all of its child states are
 non-final or there exists a successor over the zero tracks that is final (due
 to projection). During the generating of successor we can prune some of the
 states through the $\leq_i$ relation.
 
\begin{algorithm}[h!]
		\SetKwFor{For}{foreach}{do}{}
		\SetKwRepeat{Repeat}{repeat}{until}
		\KwIn{State $q$ of automaton $A_{\varphi_m}$, level of determinization $m$}
		\KwOut{\textsc{TRUE} if state $q$ is final, \textsc{FALSE} otherwise}
		\KwData{\texttt{OracleCache} caches the answers of \texttt{StateIsFinal}
		function}
		\BlankLine
		\nl finality := OracleCache[q]\;
		\nl \uIf{finality $\neq$ $\bot$}{
		  \nl\Return finality
		}
		\BlankLine
		...
		\nl \Else{
			\nl\For{$q \in state$}{
				\nl \uIf{StateIsFinal(q, m-1)}{
					\nl finality := \textsc{FALSE}\;
					\nl \textbf{break}\;
				}
				\nl \Else{
					\nl succ := buildSuccessorTree(q)\;
					\nl \textbf{apply1} succ ($\lambda$ x. \textbf{if} x is reachable by
					low successors in BDD \textbf{then} workset := workset $\cup$ x) 
				}
			}
			\nl finality := \textsc{TRUE}\;
		}
		\nl OracleCache[q] = finality\;
		\nl \Return finality\;
		\caption{Deciding whether state is
		final or not}\label{impl-state-is-final}
	\end{algorithm}
 
 The main algorithm \ref{impl-main} is based on worklist algorithm. Since we
 are working with closed formulae, at the last level of determinization in
 family of defined automata we are looking for a non-final state using the
 function \texttt{StateIsFinal} as described on \ref{impl-state-is-final}. If
 the initial state is not final, we are generating its next successor and add it
 to worklist if it is not enqueued already or we can prune it through the
 $\leq_i$ relation.
...
\begin{algorithm}[h!]
		...
		\nl \While{$\exists q_m \in workset$}{
			...
			\nl\uIf{not StateIsFinal($q_m$, level)}{
				\nl \Return \textsc{TRUE}\;
			}
			\nl\Else{
				\nl succ := buildSuccessorTree($q_m$, level)\;
				\nl\textbf{apply1} succ ($\lambda$ x. \textbf{if} x $\notin$ processed
				\textbf{then} workset := workset $cup \{x\}$)\;
				processed := processed $\cup$ workset\; 
				}
		}
		\caption{Deciding whether there exists a satisfying example for formula	
		}\label{impl-main}
	\end{algorithm}
 
 The successor of a state is constructed out of its children \ref{impl-succ}.
 The construction is optimized with the use of cache. After the BDDs for
 transitions of children of a state are build we use the binary apply for doing
 the union of those BDDs to yield the BDD corresponding to the transition for
 the state. This BDD is further projected using the operation of projection. 
...
\begin{algorithm}[h!]
		...
		\KwData{\texttt{BDDCache} mapping macrostates to its transition BDDs}
		\BlankLine
		\nl succ := BDDCache[q]\;
		\nl \uIf{succ $\neq \bot$}{
			\nl \Return succ\;
		...
		\nl succ := $\emptyset$\;
		\nl\For{$r \in q$}{
			\nl childSucc := \texttt{buildSuccessor(r, m-1)}\;
			\nl \textbf{apply2} succ childSucc ($\lambda$ x y. x $\cup$ y)\;
		}
		\nl succ := project(succ, level)\;
		\nl BDDCache[q] := succ\;
		\nl \Return succ\;
		\caption{Building successor of state $q$}\label{impl-succ}
	\end{algorithm}
 
 \section{Optimizations}
 
 We used the tool \texttt{valgrind}, especially its part \texttt{callgrind}, to
 profile the implementation and tried to identify weak spots of the application.
 Of the most frequent operation in the procedure is the relational operators on
 the classes representing macro-states used f.e. for pruning the state search,
 macro-state and operations with worklist.
 ...
 \ref{atomic-formulae} we can heavily reduce the size of matrix automaton by
 extending the set of the atomic formulae defined in \ref{restricted} and yield
 better computational results.
 
 {\color{red} Maybe do a Appendix with all the atomic formulae?}
...
studied the impact of the usage of non-determinsitic automata on two case
...
optimizations, we have shown that the non-determinitic approach to deciding
...
\textsc{MONA} always does minimizations after every operation so it works with
...
Direct Acyclic Graph (DAG) \cite{dag} instead of plain AST. 
