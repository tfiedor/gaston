...
\begin{function}[h!]
 ...
 \end{equation} 
 which corresponds to automaton with 13 states. However we can construct the
...
\end{figure}
 ...
 lessening the number of projections produced by creation of some temporary
...
12.10 with 4096\ MB of operating memory and one virtual processor natively
running on a laptop computer with a 8\ GB RAM memory and dual core processor running on 2.5
GHz.
...
approach and is heavily optimized (See \ref{monasecrets}).
...
insufficient memory or it has been interupted due to the time limit and thus we
could not recieve any valid data.
...
manipulating non-deterministic automata \cite{libvata} is far more memory
efficient with handling MTBDDs and has no problem deciding these formulae and
fairly compete with the proposed logical composition approach.
...
automata that are stored as optimization and not flaw in automata-based
approach, since our tool \texttt{dWiNA} has no problems with deciding formulae for greater $n$.
...
\begin{center}
 ...
 \cline{2-4}
  \textbf{n} & \textbf{logic} & \texttt{dWiNA} & \texttt{MONA}\\
  \hline
  10 & 0.01 & 0.01 & 0.12\\
  12 & 0.01 & 0.01 & 0.89\\
  13 & 0.01 & 0.01 & 2.28\\
 ...
 dependence on paramter $n$ for fixed number of alternations (1).}\label{n=1}
 \end{center}
\end{table}
...
\begin{figure}[h!]
 ...
 \end{center}
 \caption{Comparison of speed of \texttt{dWiNA} and \textsc{MONA} based on
...
parameter $n$ and number of alternation. It is clear from Figure \ref{alt-n}
that due to the huge differences in base automaton size between odd and even
alternations on generated benchmark formuale the time needed for computation is
...
much time to compute, like pruning or comparison. \textsc{MONA} however has no
problem with alternations and yields steady times for all numbers of
alternations showing that full construction of automata along with minimizations
is still efficient and has good results in product. This is contrary to our base
...
cannont be done and were thus approximated in shown mesh grid.
...
Even though \textsc{MONA} is more consistent when it comes to number of
...
is great asset to the overall speed.
...
\catcode`\-=12
 \begin{center}
  ...
  \hline
   ...
   [s]} & \textbf{pruning [s]} & \textbf{smart flatten} & \textbf{cache}\\
   \hline
    \multirow{4}{*}{5} & 2 & 0.11 & $\infty$ & 24.05 & 0.49 & 25.2\\
    \cline{2-7}
     & 3 & 0.02 & 0.75 & 0.16 & 0.04 & 0.08\\
    \cline{2-7}
     & 4 & 0.14 & $\infty$ & $\infty$ & 1.09 & 13.71\\
    \cline{2-7}
     & 5 & 0.03 & 5.4 & 1.04 & 0.17 & 0.42\\
    \hline
   \hline
   \multirow{4}{*}{6} & 2 & 2.2 & $\infty$ & $\infty$ & 9.59 & $\infty$\\
   \cline{2-7}
    & 3 & 0.01 & 1.22 & 0.3 & 0.06 & 0.14\\
    \cline{2-7}
  ...
  \end{tabular}
 \end{center}
 \caption{Speed compare for various optimizations based on number of
...
\begin{table}[h!]
 \begin{center}
  ...
  \end{tabular}
 \end{center}
 \label{Average cache-hit ration for various level of determinizations for
...
\textsc{MONA} in several aspects, namely generated search space and computation
time to decide benchmark formulae for some cases. While \textsc{MONA} is
...
contrary to classical deterministic approach. While \textsc{MONA} is more
...
We further propose some further optimizations that could
